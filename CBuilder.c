...
void CompileFilesNULL(const char *dest, const char *compiler, ...)
{
    va_list args;
    va_start(args, compiler);
    ...
    while ((pattern = va_arg(args, const char *)) != NULL)
    {
        ...
        for (int i = 0; i < fileCount; ++i)
        {
            ...
            va_copy(compilerArgs, args);
            const char *compilerArg;
            while ((compilerArg = va_arg(compilerArgs, const char *)) != NULL && compilerArg[0] == '-')
            {
                AppendToCommand(cmd, compilerArg);
            }
            va_end(compilerArgs);
...
#define CompileFiles(out, cc, ...) CompileFilesNULL(out, cc, __VA_ARGS__, NULL)

void LinkFilesNULL(const char *outputBinary, const char *linker, ...)
{
    va_list args;
    ...
    while ((arg = va_arg(args, const char *)) != NULL)
    {
        ...
        else
        {
            ...
            RecursiveCollectFiles(arg, files, &fileCount, 1000);
...
#define LinkFiles(out, ld, ...) LinkFilesNULL(out, ld, __VA_ARGS__, NULL)
 ...
 * ----------------------------------------- USER SPACE ----------------------------------------- */
	...
	#define EXECNAME	"./bscript"
	#define CC			DEFAULT_COMPILER
	#define LD			"clang"
	#define CFLAGS		"-std=c99 -Wall -Wextra -Werror -pedantic-errors -O0"
	#define LDFLAGS		""
	#define DEBUG_CFLAGS	"-fsanitize=address,undefined -g3"
	#define DEBUG_LDGLAGS	"-fsanitize=address,undefined"
...
void BuildRule(int argc, char **argv)
{
	...
	{
		strcat(cflags, DEBUG_CFLAGS);
		strcat(ldflags, DEBUG_LDGLAGS);
	}
	CompileFiles("build", CC, "src/*.c", "src/crypto/*.c", cflags);
	LinkFiles(EXECNAME, LD, ldflags, "build/*.o");
